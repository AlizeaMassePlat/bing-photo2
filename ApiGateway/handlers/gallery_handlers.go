...
// @Summary Déplacer un média vers un autre album
// @Description Déplace un média d'un album source vers un album cible
// @Tags Media
// @Accept json
// @Produce json
// @Param id path int true "ID du média à déplacer"
// @Param move body proto.MoveMediaRequest true "Données de déplacement"
// @Success 200 {object} proto.MoveMediaResponse
// @Failure 400 {string} string "Requête invalide"
// @Failure 401 {string} string "Authorization header missing"
// @Failure 500 {string} string "Erreur serveur"
// @Router /media/{id}/move [post]
// @Security BearerAuth
func (g *GalleryGateway) MoveMediaHandler(w http.ResponseWriter, r *http.Request) {
	// Auth
	authHeader := r.Header.Get("Authorization")
	if authHeader == "" {
		http.Error(w, "Authorization header missing", http.StatusUnauthorized)
	...
	// Body JSON
	var req proto.MoveMediaRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "Invalid request payload", http.StatusBadRequest)
		return
	}
	req.MediaId = uint32(mediaID) // On force l'ID du média depuis l'URL
