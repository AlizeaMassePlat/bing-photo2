...
func (s *AlbumService) DeleteAlbum(albumID uint) error {
	...
	// Supprimer les médias associés à l'album dans la base de données
	err = s.DBManager.DB.Where("album_id = ?", albumID).Delete(&models.Media{}).Error
	if err != nil {
		return fmt.Errorf("échec de la suppression des médias de l'album : %v", err)
	}
