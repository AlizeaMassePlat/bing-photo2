...
type MoveMediaRequest struct {
    TargetAlbumID uint `json:"target_album_id"`
}
...
func (h *MediaHandler) MoveMedia(w http.ResponseWriter, r *http.Request) {
    ...
    mediaID, err := strconv.ParseUint(vars["id"], 10, 64)
    if err != nil {
        http.Error(w, "ID de média invalide", http.StatusBadRequest)
        return
    }
    ...
    w.WriteHeader(http.StatusOK)
    json.NewEncoder(w).Encode(map[string]string{"message": "Média déplacé avec succès"})
}
